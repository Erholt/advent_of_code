...
test.describe("Day 2", () => {
  ...
  test.describe("Solution 2", () => {
    test("solution_2 returns 34", () => {
      assert.strictEqual(solution_2(["2x3x4"]), 34);
    });
  ...
  test.describe("Gift Object", () => {
    test.describe("wrapping paper", () => {
      test("returns correct amount for 2x3x4", () => {
        assert.strictEqual(gift("2x3x4").wrapping_paper, 58);
      });

      test("returns correct amount for 1x1x10", () => {
        assert.strictEqual(gift("1x1x10").wrapping_paper, 43);
      });
    });

    test.describe("ribbon", () => {
      test("returns correct amount for 2x3x4", () => {
        assert.strictEqual(gift("2x3x4").combined_ribbon, 34);
      });

      test("returns correct amount for 1x1x10", () => {
        assert.strictEqual(gift("1x1x10").combined_ribbon, 14);
      });
    });
  });
});
